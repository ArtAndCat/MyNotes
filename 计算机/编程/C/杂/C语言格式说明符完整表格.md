# 字符样式
- `\033` 是八进制的 ESC 字符（ASCII 27），表示转义序列开始
- `[` 是控制序列引入符
- `31` 是参数，表示红色
- `m` 表示设置图形模式（颜色、样式等）
常见颜色代码：
...
例：
```cpp
cout(tw,"\033[31m 错误！游戏已经运行过一遍了！",1,900);
```
这行代码会在终端中显示红色的错误信息。
### 其他常用样式：
- `\033[0m` - 重置所有属性（恢复正常颜色）
- `\033[1m` - 粗体/高亮
- `\033[4m` - 下划线
...
```cpp
// 红色文本
cout(tw,"\033[31m这是红色文字",1,900);

// 粗体绿色文本  
cout(tw,"\033[1;32m这是粗体绿色文字",1,900);

// 恢复正常颜色
cout(tw,"\033[0m这是正常颜色",1,900);
```

这种转义序列在命令行程序、终端应用中常用于美化输出和错误提示。
